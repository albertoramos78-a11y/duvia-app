...
- Météo : déjà fetchée côté client directement depuis Open-Meteo (API publique, sans clé). **Point de sécurité important découvert en écrivant le plan** : `parent_locations` (migration `0035_parent_locations.sql`) n'a délibérément AUCUNE policy RLS "famille entière" — un parent ne peut lire QUE sa propre ligne (`user_id = auth.uid()`), les coordonnées d'un co-parent sont sensibles (sécurité domestique). La fonction déjà livrée `get-family-weather` (`docs/superpowers/specs/2026-07-13-weather-location-privacy-design.md`) gère déjà ce cas : vérifie l'appartenance familiale avec le JWT de l'appelant, puis lit `parent_locations` avec le client service-role UNIQUEMENT pour ça, et ne renvoie jamais que les champs dérivés (code météo, températures) — jamais lat/lon/ville. L'outil `get_weather` du chatbot DOIT suivre exactement ce même modèle (pas un simple client JWT-scopé comme les 3 autres outils) pour permettre au chatbot de répondre sur la météo de CHAQUE parent sans réintroduire la fuite que cette RLS restrictive corrige.
...
- **`get_weather(days?)`** — vérifie l'appartenance familiale via le client JWT-scopé, puis lit `parent_locations` de CHAQUE parent de la famille via le client service-role (seule exception à la règle "toujours JWT-scopé", voir Contexte technique), appelle Open-Meteo pour chaque ville (même endpoint que le client, voir `fetchMyWeatherForecast` dans `App.jsx`), retourne UNIQUEMENT les prévisions résumées par jour (jamais lat/lon/ville) pour chaque parent, étiquetées "vous"/"votre co-parent".
...
- Chaque outil exécute sa requête avec le JWT de l'appelant, jamais la clé service-role — les règles RLS déjà en vigueur pour ce compte/rôle s'appliquent automatiquement, sans logique de permission à dupliquer ni à laisser dériver du reste de l'app. **Seule exception** : `get_weather`, qui utilise le client service-role pour lire `parent_locations` des AUTRES parents (RLS volontairement restreinte à `user_id = auth.uid()`, voir Contexte technique) — en suivant exactement le modèle déjà audité de `get-family-weather`, jamais lat/lon/ville ne sortant de l'outil.
