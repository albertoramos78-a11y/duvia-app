...
// ═══════════════════════════════════════════════════════════════════════════════
const _SUPA_URL = ""; // ← "https://xxx.supabase.co"
const _SUPA_KEY = ""; // ← "eyJhbGci..."
...
// ─── localStorage SHIM ────────────────────────────────────────────────────────
// ⚠️  À SUPPRIMER EN PRODUCTION (Vercel/Supabase) — sandbox Claude uniquement.
// En prod, le vrai localStorage du navigateur sera utilisé automatiquement.
(function () {
  const mem = {};
  const shim = {
  ...
  Object.defineProperty(shim, "length", { get: () => Object.keys(mem).length });
  try {
  ...
  try {
    ...
    try { window.sessionStorage = shim; } catch (e2) {}
  }
})();
...
const TR = {
  fr:{
    ...
    forgotPw:"Mot de passe oublié ?",backLogin:"← Retour",
    ...
    stepId:"Identifiants",stepDates:"Dates spéciales",stepGarde:"Modèle garde",stepAccess:"Accès",
    ...
    grandparent:"Grand-parent",uncleAunt:"Oncle / Tante",otherFamily:"Autre famille",
  ...
  en:{
    ...
    forgotPw:"Forgot password?",backLogin:"← Back",
    ...
    stepId:"Identities",stepDates:"Special dates",stepGarde:"Custody pattern",stepAccess:"Access",
    ...
    grandparent:"Grandparent",uncleAunt:"Uncle / Aunt",otherFamily:"Other family",
  ...
  de:{
    ...
    forgotPw:"Passwort vergessen?",backLogin:"← Zurück",
    ...
    stepId:"Identitäten",stepDates:"Besondere Daten",stepGarde:"Sorgerechtsmodell",stepAccess:"Zugang",
    ...
    grandparent:"Großelternteil",uncleAunt:"Onkel / Tante",otherFamily:"Andere Familie",
  ...
  es:{
    ...
    forgotPw:"¿Olvidaste tu contraseña?",backLogin:"← Volver",
    ...
    stepId:"Identidades",stepDates:"Fechas especiales",stepGarde:"Modelo custodia",stepAccess:"Acceso",
    ...
    grandparent:"Abuelo/a",uncleAunt:"Tío/a",otherFamily:"Otra familia",
  ...
  pt:{
    ...
    forgotPw:"Esqueceu a senha?",backLogin:"← Voltar",
    ...
    stepId:"Identidades",stepDates:"Datas especiais",stepGarde:"Modelo guarda",stepAccess:"Acesso",
    ...
    grandparent:"Avô/Avó",uncleAunt:"Tio/Tia",otherFamily:"Outra família",
...
// ─── DATA ─────────────────────────────────────────────────────────────────────
const DEMO_USERS = [
  {id:0,email:"admin@demo.fr",password:"admin2024",name:"Administrateur",role:"admin"},
  {id:1,email:"parent1@demo.fr",password:"demo123",name:"Marie Dupont",role:"parent",parentIdx:0},
  {id:2,email:"parent2@demo.fr",password:"demo123",name:"Jean Dupont",role:"parent",parentIdx:1},
  {id:3,email:"mamie@demo.fr",password:"demo123",name:"Mamie Suzanne",role:"observer"},
  {id:4,email:"enfant@demo.fr",password:"demo123",name:"Léa Dupont",role:"child"},
];
...
export default function App() {
  ...
  if(!user) return (
    <div>
      ...
      {pendingUser ? (
        ...
        <LoginScreen C={C} t={t} lang={lang} setLang={setLang} themeMode={themeMode} cycleTheme={cycleTheme} users={users} setUsers={setUsers} onLogin={handleLogin} onObsJoin={handleObsJoin} />
      )}
    </div>
  );
  ...
  return (
    ...
    <div style={{display:"flex",flexDirection:"column",height:"100vh",maxWidth:940,margin:"0 auto",overflow:"hidden",width:"100%"}}>
      ...
      {menuTab==="config" ? (
        (() => {
          const STEPS=[{i:"👤",l:t.stepId},{i:"🗓️",l:t.stepDates},{i:"📆",l:t.stepGarde},{i:"👥",l:t.stepAccess},{i:"🌐",l:t.stepLang||"Langue"}];
          return (
            <div style={{flexShrink:0,background:C.card,borderBottom:`1.5px solid ${C.bor}`,boxShadow:"0 1px 6px rgba(0,0,0,.05)"}}>
              ...
              <div style={{display:"flex",alignItems:"center",gap:10,padding:"8px 12px 6px"}}>
                ...
                {configStep===1 && <StepDatesInfoButton C={C} t={t} />}
                {configStep===2 && <StepGardeInfoButton C={C} t={t} />}
                {configStep===3 && <StepAccessInfoButton C={C} t={t} />}
              </div>
              {/* Onglets étapes */}
      ...
      {isBeta() && user && !sub._admin && (
        <div style={{
          ...
          <div style={{fontSize:10,color:C.mut,fontWeight:600}}>
            {(t.daysLeftSuffix||"{n}j restants").replace("{n}",BETA_DAYS_LEFT())}
          </div>
        </div>
      )}
...
function LoginScreen({C,t,lang,setLang,themeMode,cycleTheme,users,setUsers,onLogin,onObsJoin}) {
  ...
  function doLogin(){const u=users.find(u=>u.email===email&&u.password===pw);if(!u){setErr(t.wrongPw);return;}onLogin(u);}
    ...
    <div style={{minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",padding:20,background:C._brand?`linear-gradient(rgba(255,255,255,.6),rgba(255,255,255,.6)),linear-gradient(145deg,#7BA8F5 0%,#9D8FF0 26%,#F8F2FF 52%,#FF9FD2 76%,#FF6BB5 100%)`:`linear-gradient(rgba(255,255,255,.6),rgba(255,255,255,.6)),radial-gradient(ellipse at 30% 20%,rgba(124,111,205,.15) 0%,transparent 60%),${C.bg}`}}>
      <div style={{width:"100%",maxWidth:400}} className="fi">
        ...
        <div className="card" style={C._brand?{background:`linear-gradient(rgba(255,255,255,.5),rgba(255,255,255,.5)),linear-gradient(145deg,#7BA8F5 0%,#9D8FF0 26%,#F8F2FF 52%,#FF9FD2 76%,#FF6BB5 100%)`}:undefined}>
              ...
              {showExistingAccount && (
                <div style={{background:`${C.yel}12`,border:`1.5px solid ${C.yel}55`,borderRadius:12,padding:"14px 16px",marginBottom:16}}>
                  ...
                  <input
                    ...
                    autoFocus
                    style={{width:"100%",height:44,boxSizing:"border-box",fontSize:14,marginBottom:12}}
                  />
          ...
          {mode!=="forgot"&&<div className="field"><label className="lbl">{t.password}</label><input type="password" value={pw} onChange={e=>setPw(e.target.value)} onKeyDown={e=>e.key==="Enter"&&mode==="login"&&doLogin()} /></div>}
          ...
          <div style={{marginTop:16,paddingTop:16,borderTop:`1.5px solid ${C.bor}`}}>
            <div style={{fontSize:10,color:C.mut,textAlign:"center",marginBottom:12,fontWeight:800,letterSpacing:".1em",textTransform:"uppercase"}}>{t.demoAccounts}</div>
            {DEMO_USERS.map(u=>(
              <button key={u.id} onClick={()=>{setEmail(u.email);setPw(u.password);setMode("login");}} style={{width:"100%",height:44,padding:"0 12px",background:u.role==="admin"?"linear-gradient(135deg,#fff8dc,#fff3cc)":C.sur,border:`1.5px solid ${u.role==="admin"?"#fcd34d":C.bor}`,color:C.txt,fontSize:12,textAlign:"left",display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:6,borderRadius:10}}>
                <span style={{fontWeight:u.role==="admin"?900:700,display:"flex",alignItems:"center",gap:6}}>{u.role==="admin"?"👑":u.role==="observer"?"👁️":u.role==="child"?"🧒":"👤"} {u.name}</span>
                <span style={{color:u.role==="admin"?"#f97316":u.role==="observer"?C.ora:u.role==="child"?C.grn:C.vio,fontWeight:800,fontSize:10}}>
                  {u.role==="admin"?"✦ Admin":u.role==="observer"?t.roleObs:u.role==="child"?(t.roleChild||"🧒 Enfant"):t.roleParent}
                </span>
              </button>
            ))}
          </div>
...
function ConfigTab() {
  ...
  function confirmInvite(){
    ...
    const inviteUrl = `https://duvia.app/join?code=${code}&role=parent&family=${cfg.shareCode||"DUVIA"}`;
...
function ChildInviteBtn({ childIdx, childName, childPhone }) {
  ...
  function getOrGenUrl() {
    ...
    const url  = `https://duvia.app/join?code=${code}&role=child&family=${cfg.shareCode||"DUVIA"}&idx=${childIdx}`;
    setCfg(c => ({
      ...c,
      pendingChildInvites: [
...
function CustomSelect({value, onChange, options, style}) {
  ...
  return (
    <div style={{position:"relative",...style}}>
      ...
      {open && (
        <div style={{position:"absolute",[openUp?"bottom":"top"]:openUp?"calc(100% + 6px)":"calc(100% + 6px)",left:0,right:0,background:C.card,border:`1.5px solid ${C.bor}`,borderRadius:16,zIndex:300,boxShadow:"0 12px 40px rgba(0,0,0,.2)",overflow:"hidden",maxHeight:280,overflowY:"auto"}}>
          {options.map((o,i)=>{
...
function StepAccess() {
  ...
  const rl={grandparent:t.grandparent,"uncle-aunt":t.uncleAunt,other:t.otherFamily};
  ...
  function sendInvite(){
    ...
    const inviteUrl=`https://duvia.app/join?code=${code}&role=observer&family=${cfg.shareCode}`;
      ...
      <div className="card" style={{marginBottom:16,position:"relative"}}>
        ...
        {!sent?(
          ...
          <div>
            ...
            <button onClick={()=>{setSent(false);setCopied(false);}} style={{width:"100%",height:38,background:C.sur,color:C.mut,border:`1.5px solid ${C.bor}`,fontSize:13}}>{t.addObsBtn} →</button>
          </div>
        )}
      </div>
    ...
    <div>
      ...
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:16}}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:8,flexShrink:0}}>
          ...
          <InfoBubble C={C} tipKey={`duvia_caltip_${user?.id||"x"}`} title="Calendrier" autoOpen={false}>
            Visualisez et gérez le planning de garde mensuel. Il est visible par tous les membres de la famille.
          </InfoBubble>
        </div>
      </div>
      {/* Sélecteur d'enfant */}
      ...
      {(() => {
        ...
        return (
          <button onClick={()=>{ setMenuTab("config"); setConfigStep(2); }}
            style={{
      ...
      {!isPremium&&(
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:14}}>
          {[{cy:"monthly",pr:t.monthly,save:null},{cy:"yearly",pr:t.yearly,save:t.yearlyNote}].map(p=>(
            <div key={p.cy} className="card" style={{borderColor:p.cy==="yearly"?C.vio:C.bor,textAlign:"center",padding:"16px 10px",position:"relative",opacity:isBeta()?0.6:1}}>
              ...
              <div style={{fontSize:22,fontWeight:900,color:C.vio}}>{p.pr}</div>
              <div style={{fontSize:11,color:C.mut,marginBottom:10}}>{t.perFamily}</div>
              {isBeta() ? (
...
function MessagingTab(){
  ...
  const contacts=(users||[]).filter(u=>String(u.id)!==myId&&u.name);
