...
- **Two Supabase projects, confusingly named** — the dashboard names do NOT match their real role: production (the one `app.duvia.fr` actually calls, verified by inspecting the deployed JS bundle's baked-in URL) is the project named **"DUVIA-RMS-DEV"** (ref `ifhriyvvqkwqgzmrjjxp`), NOT the one named "duvia-staging". The project actually named **"duvia-staging"** (ref `xqborcugpzjzungwgepn`) is what the local `.env` points to for `npm run dev` — genuinely staging, just named confusingly close to the real prod project's misleading name. Always double check with `supabase projects list` (or by grepping the live bundle for `.supabase.co`, see `git log` around 2026-07-22) rather than trusting a project's dashboard name.
...
### Not yet done
The duplicate-Supabase-client concern was investigated (2026-07-10) and is **not** a real gap: only one `createClient()` call exists client-side (`src/supabaseClient.js`), imported consistently everywhere, and only one `@supabase/supabase-js` version resolves in the dependency tree. (A since-deleted dead file, `localStorageMigration.ts`, imported a non-existent second `supabaseClient` path but was never itself imported by anything, so it never actually ran.) Bad-word filter's naive substring matching was fixed for common French/English/German/Spanish/Portuguese false positives, but "pedale"/"pedales" (French bicycle pedal vs. homophobic slur, identical word) was deliberately left as-is pending a product decision. "Passwords not hashed client-side" was re-evaluated and is **not** a real gap (standard practice — Supabase Auth hashes server-side). `expire_stale_family_data()` is now scheduled via `pg_cron` (daily, see migration history / Supabase dashboard cron jobs — not committed as a migration file in this repo). Similarly, `notify-bug-report`'s trigger (a Database Webhook on `bug_reports` INSERT) and its `BUG_REPORT_WEBHOOK_SECRET` are configured only in the Supabase dashboard — not captured as a migration file in this repo.

**Edge Functions now deploy via the Supabase CLI, not dashboard copy-paste (fixed 2026-07-22).** Previously, functions were deployed by pasting code directly into the Supabase dashboard editor, with no CLI/CI link back to this repo — the committed source silently went stale twice (`delete-account`, found/fixed 2026-07-08; `notify-expense`/`notify-message`/`notify-vault`, found 2026-07-08, see `docs/superpowers/plans/2026-07-08-push-notifications.md` and commit `4cb0038`) and two functions (`admin-backup-manager`, `backup-upload`) were never committed here at all until recovered from prod on 2026-07-22. This is now resolved: the repo is the single source of truth, deployed with `npx supabase functions deploy [name] --use-api` (the `--use-api` flag bundles server-side, no local Docker needed). No build step or CLI install required beyond `npx` — see `supabase/config.toml` (created by `supabase init`, committed). To deploy: `npx supabase login` once (opens a browser; stores the CLI token locally, not in this repo), `npx supabase link --project-ref ifhriyvvqkwqgzmrjjxp` once per machine, then `npx supabase functions deploy <name>` (or with no name to deploy all) after any Edge Function edit. **Do not revert to dashboard paste-editing** — a manual dashboard edit made outside this flow would silently reintroduce the same drift risk this migration fixed.
