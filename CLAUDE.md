...
- `src/config.js` — global constants: `LIMITS` (validation caps), `RGPD_NOTICE_VERSION` (bump this to force re-consent — it's compared against the timestamped record in `duvia_rgpd_consent`), legal URLs (currently placeholders pointing at the marketing site).
...
- RGPD consent age threshold is hardcoded to 15 (France); note in `BUGFIXES.md` that other countries legally differ (DE 16, ES 14, PT 13) and a per-country threshold is a known follow-up, not yet implemented.

### Not yet done (from `BUGFIXES.md`, still relevant)
Bad-word filter does naive substring matching (false positives on long words). Passwords are still not hashed client-side before reaching Supabase Auth. `expire_stale_family_data()` exists but isn't scheduled. There's a known duplicate-Supabase-client concern flagged for follow-up.
