...
function useFamilySync(cfg, setCfg) {
  ...
  // ── Créer un compte Supabase Auth (source de vérité) ───────────────────
  // Utilisé à l'inscription. Supabase Auth = seul endroit qui fait foi.
  // Prérequis : Authentication > Providers > Email > "Confirm email" DÉSACTIVÉ
  async function linkAccount(email, password, metadata) {
    try {
      ...
      // "User already registered" n'est pas une erreur bloquante
...
export default function App() {
  ...
  // Admin exempté (compte local uniquement).
  useEffect(() => {
    if (!sessionEmail) return;
    const isAdmin = DEMO_USERS.some(u => u.role === "admin" && u.email === sessionEmail);
    if (isAdmin) return;
    (async () => {
      try {
        const { data: sessData } = await supabase.auth.getSession();
...
function LoginScreen({C,t,lang,setLang,themeMode,cycleTheme,users,setUsers,onLogin,onObsJoin,familySync}) {
  ...
  async function doLogin(){
    ...
    // Admin : compte local uniquement, ne passe pas par Supabase
    const adminUser = users.find(u => u.role==="admin" && u.email===cleanEmail && u.password===pw);
    if(adminUser){ onLogin(adminUser); return; }

    // Tous les autres : Supabase Auth est la seule source de vérité
    setErr(""); setOk(t.syncConnecting||"Connexion…");
    const res = await familySync.signInExisting(cleanEmail, pw);
    if(!res.ok){
      // Supabase a refusé → on supprime le cache local pour cet email
      setUsers(us => us.filter(u => u.email !== cleanEmail));
      setOk(""); setErr(t.wrongPw); return;
    }

    // Supabase OK → on reconstruit le profil local depuis les métadonnées Supabase
    const meta = res.metadata || {};
    const existing = users.find(u => u.email===cleanEmail);
    const profile = existing
      ? { ...existing, name: existing.name || meta.name || cleanEmail.split("@")[0] }
      : {
  ...
  async function doReg(){
    ...
    // Vérification email : Supabase Auth fait foi, pas le localStorage
    // On tente un signUp → Supabase répond "already registered" si le compte existe
    {
      const { error: preCheck } = await supabase.auth.signUp({
        email: cleanEmail, password: "duvia_probe_check_99z",
        options: { data: {} },
      });
      const alreadyInSupabase = preCheck?.message?.includes("already registered");
      if (alreadyInSupabase) {
        // Email déjà dans Supabase → vrai doublon
        if(isParentInvite){ setMode("login"); setErr("Ce compte existe déjà. Connectez-vous pour rejoindre la famille."); return; }
        if(isAnyInvite){ setShowExistingAccount(true); setErr(""); return; }
        setErr(t.emailUsed); return;
      }
      // Pas dans Supabase → on nettoie le cache localStorage si besoin et on continue
      setUsers(us => us.filter(u => u.email !== cleanEmail));
    }
