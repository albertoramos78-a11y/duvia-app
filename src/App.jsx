...
function useFamilySync(cfg, setCfg) {
          ...
          try {
            ...
            try {
              ...
              if (me) {
                ...
                // Dans sa nouvelle famille, l'utilisateur est le parent unique (index 0).
            ...
            // Rechargement unique : au prochain boot, l'adhésion active (nouvelle
            // famille) est trouvée et tout se charge proprement avec parentIdx=0.
  ...
  async function leaveFamily() {
    if (!familyIdRef.current) return { ok: false, error: "no-family" };
    try {
      ...
      const { error } = await supabase.rpc("leave_family", { p_family_id: familyIdRef.current });
      if (error) throw error;
      return { ok: true };
    } catch (e) {
  ...
  if(mode==="obs_waiting") return (
    <div style={{minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",padding:20,background:C.bg}}>
      <div style={{background:C.card,borderRadius:20,padding:"30px 24px",maxWidth:420,width:"100%",textAlign:"center",boxShadow:"0 10px 40px rgba(0,0,0,.12)"}}>
        ...
        <button onClick={()=>setMode("login")} style={{height:42,padding:"0 22px",background:C.sur,color:C.mut,border:`1.5px solid ${C.bor}`,fontSize:13,borderRadius:10}}>← {t.backLogin||"Retour"}</button>
      </div>
    </div>
  );
        ...
        <div className="card" style={C._brand?{background:`linear-gradient(rgba(255,255,255,.5),rgba(255,255,255,.5)),linear-gradient(145deg,#7BA8F5 0%,#9D8FF0 26%,#F8F2FF 52%,#FF9FD2 76%,#FF6BB5 100%)`}:undefined}>
          ...
          {(mode==="login"||mode==="register")&&(
            <label style={{display:"flex",alignItems:"center",gap:8,margin:"2px 0 12px",cursor:"pointer",fontSize:12.5,color:C.mut}}>
              <input type="checkbox" defaultChecked={(()=>{try{return window.localStorage.getItem("duvia_remember")==="1";}catch{return false;}})()}
                onChange={e=>{ try{ if(e.target.checked) window.localStorage.setItem("duvia_remember","1"); else window.localStorage.removeItem("duvia_remember"); }catch{} }}
  ...
  return (
    <div>
      ...
      {cfg.parents.map((p,i)=>{
        ...
        const isMine = i === user?.parentIdx; // je n'édite/vois en détail que MA fiche
