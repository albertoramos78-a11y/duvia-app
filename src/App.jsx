...
function useFamilySync(cfg, setCfg) {
  ...
  // ── Créer un vrai compte Supabase Auth (email + mot de passe) ──────────
  // On essaie d'abord de convertir la session anonyme courante (updateUser).
  // Si ça échoue (pas de session anonyme, ou session d'un autre compte),
  // on crée directement avec signUp — c'est la vraie source de vérité.
  async function linkAccount(email, password, metadata) {
    try {
      // Tentative 1 : convertir la session anonyme courante
      const { data: sessData } = await supabase.auth.getSession();
      const isAnon = sessData?.session?.user?.is_anonymous === true;
      if (isAnon) {
        const { error } = await supabase.auth.updateUser({
          email, password, data: metadata || {},
        });
        if (!error) return { ok: true };
        // Si updateUser échoue, on tombe sur signUp ci-dessous
        console.warn("[Duvia] updateUser failed, fallback to signUp:", error.message);
      }
      // Tentative 2 : signUp direct (nouvel appareil, pas de session anonyme)
      const { error: signUpErr } = await supabase.auth.signUp({
        email, password,
        options: { data: metadata || {} },
      });
      if (signUpErr) {
        // "User already registered" → le compte existe déjà, ce n'est pas une erreur
...
export default function App() {
  ...
  // Les admins (compte local uniquement) sont exemptés de cette vérification.
  useEffect(() => {
    if (!sessionEmail) return;
    // Admin → local uniquement, pas de vérification Supabase
    const adminUser = DEMO_USERS.find(u => u.role === "admin" && u.email === sessionEmail);
    if (adminUser) return;
    (async () => {
      try {
        const { data: sessData } = await supabase.auth.getSession();
        if (!sessData?.session) {
          // Pas de session Supabase valide → déconnexion + nettoyage cache
          setUser(null);
          setSessionEmail(null);
        }
      } catch {
        // Hors ligne → on laisse la session locale active (mode offline)
      }
    })();
...
function LoginScreen({C,t,lang,setLang,themeMode,cycleTheme,users,setUsers,onLogin,onObsJoin,familySync}) {
  ...
  async function doLogin(){
    ...
    // ── Compte admin (local uniquement, pas dans Supabase Auth) ──────────
    const adminUser = users.find(u => u.role==="admin" && u.email===cleanEmail && u.password===pw);
    if(adminUser){ onLogin(adminUser); return; }

    // ── Tous les autres comptes : Supabase Auth EST la source de vérité ──
    // Si le compte a été supprimé de Supabase → connexion refusée, même si
    // le localStorage a encore les données en cache.
    setErr(""); setOk(t.syncConnecting||"Connexion…");
    const res = await familySync.signInExisting(cleanEmail, pw);
    if(res.ok){
      // Supabase Auth valide → on cherche/crée le profil local
      const meta = res.metadata || {};
      const localUser = users.find(u => u.email===cleanEmail);
      if(localUser){
        // Profil local trouvé → on le met à jour avec les métadonnées Supabase
        setOk("");
        onLogin({...localUser, name: localUser.name||meta.name||cleanEmail.split("@")[0]});
        return;
      }
      // Nouvel appareil → on recrée le profil local depuis Supabase
      const newId = Date.now();
      const localU = {
        id:newId, email:cleanEmail, password:pw,
        name: meta.name || cleanEmail.split("@")[0],
        role: meta.role || "parent",
        parentIdx: meta.parentIdx,
        refCode: makeRefCode(newId, cleanEmail),
    ...
    // Supabase Auth a refusé → compte inexistant ou supprimé → on bloque
    // et on nettoie le cache local pour éviter toute confusion future
  ...
  const [obsInviteCode] = useState(()=>{
    try{
      ...
      const inv = p.get("inv");
      if(inv){
        try{ const d = JSON.parse(atob(inv)); return {code:d.code, family:d.family, role:d.role, email:d.email||""}; }catch{}
      }
  ...
  // Auto-basculer et pré-remplir email depuis le lien d'invitation
  useEffect(()=>{
    if(!isAnyInvite) return;
    const invEmail = obsInviteCode?.email || "";
    if(invEmail) setEmail(invEmail);
    if(isParentInvite && invEmail && users.find(u=>u.email===invEmail.trim().toLowerCase())){
      // Email déjà existant → écran connexion avec email pré-rempli
      setMode("login");
    } else {
      setMode("register");
    }
  ...
  async function doReg(){
    ...
    if(users.find(u=>u.email===cleanEmail)){
      if(isParentInvite){
        // Email déjà dans la base → basculer en mode connexion avec email pré-rempli
        setMode("login");
        setErr("Ce compte existe déjà. Connectez-vous pour rejoindre la famille.");
        return;
      }
      if(isAnyInvite){
    ...
    if(finalRole==="parent"){
      ...
      if(isParentInvite && obsInviteCode.family){
  ...
  async function doLoginAndJoin(){
    ...
    if(isObsInvite){
      ...
      // Compte Parent 2 existant → se connecter à Supabase puis rejoindre la famille
      const signRes = await familySync.signInExisting(cleanEmail, pw);
      if(signRes.ok){
        const joinRes = await familySync.joinFamily(obsInviteCode.family);
        if(joinRes.ok){
          // ✅ Même correctif : délai pour que skipNextSave soit remis à false avant la MAJ
          await new Promise(r => setTimeout(r, 100));
          setCfg(c=>{
            const p=[...(c.parents||[])];
            while(p.length<2) p.push({});
        ...
        console.warn("[Duvia] signInExisting failed:", signRes.error);
      }
      setShowExistingAccount(false); setErr("");
      onLogin(updatedUser);
    } else {
    ...
    <div style={{minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",padding:20,background:C._brand?`linear-gradient(rgba(255,255,255,.6),rgba(255,255,255,.6)),linear-gradient(145deg,#7BA8F5 0%,#9D8FF0 26%,#F8F2FF 52%,#FF9FD2 76%,#FF6BB5 100%)`:`linear-gradient(rgba(255,255,255,.6),rgba(255,255,255,.6)),radial-gradient(ellipse at 30% 20%,rgba(124,111,205,.15) 0%,transparent 60%),${C.bg}`}}>
      <div style={{width:"100%",maxWidth:400}} className="fi">
        ...
        <div className="card" style={C._brand?{background:`linear-gradient(rgba(255,255,255,.5),rgba(255,255,255,.5)),linear-gradient(145deg,#7BA8F5 0%,#9D8FF0 26%,#F8F2FF 52%,#FF9FD2 76%,#FF6BB5 100%)`}:undefined}>
          ...
          <div className="field"><label className="lbl">{t.email}</label><input type="email" value={email} onChange={e=>setEmail(e.target.value)} readOnly={isParentInvite && !!obsInviteCode?.email} style={isParentInvite && obsInviteCode?.email ? {background:C.sur,color:C.mut,cursor:"default"} : undefined} /></div>
...
function ConfigTab() {
  ...
  function confirmInvite(){
    ...
    const payload = btoa(JSON.stringify({ code, role: "parent", family: cfg.shareCode||"DUVIA", email: em||"" }));
