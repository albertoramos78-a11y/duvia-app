...
function useFamilySync(cfg, setCfg) {
  ...
  async function createNewFamily() {
    try {
      ...
      const blankCfg = { ...makeCfg(), shareCode: Math.random().toString(36).slice(2, 8).toUpperCase() };
...
// ─── STEP 4: ACCESS ───────────────────────────────────────────────────────────
function StepAccess() {
  const {C,t,cfg,setCfg,pushNotif,prem,perms,onUpgrade,user,familySync} = useApp();
      ...
      <div className="card" style={{marginBottom:16}}>
        ...
        <button
          ...
          onClick={async ()=>{
            ...
            const res = await familySync.createNewFamily();
