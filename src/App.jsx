...
export default function App() {
  ...
  useEffect(() => {
    ...
    // Retour depuis le cache navigateur (bfcache) : si « Rester connecté » n'est
    // pas coché, on recharge pour repartir d'un état déconnecté propre.
    const onShow = (e) => {
      try {
        if (e.persisted && window.localStorage.getItem("duvia_remember") !== "1") {
          window.location.reload();
        }
      } catch {}
    };
    window.addEventListener("pageshow", onShow);
    return () => { window.removeEventListener("pagehide", onLeave); window.removeEventListener("pageshow", onShow); };
  }, []);
