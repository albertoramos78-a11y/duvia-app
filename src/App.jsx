...
function LoginScreen({C,t,lang,setLang,themeMode,cycleTheme,users,setUsers,onLogin,onObsJoin,familySync}) {
  ...
  async function doReg(){
    ...
    // ☁️ Pour les parents : créer le compte Supabase Auth EN PREMIER
    // (linkAccount doit précéder joinFamily — joinFamily appelle getUser())
    if(finalRole==="parent"){
...
function CalTab({readOnly=false,canEdit=true,updateCal:updateCalProp}) {
  ...
  const [cur,setCur]=useState(()=>{ const today=new Date(); return new Date(today.getFullYear(),today.getMonth(),1); });
