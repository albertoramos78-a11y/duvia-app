        ...
        .on("postgres_changes",
          ...
          (payload) => {
            ...
            if (row?.user_id === uid && row?.status && row.status !== "active") {
              // Ne pas ré-afficher de message si c'est moi qui ai quitté volontairement.
...
function ConfigTab() {
  ...
  async function quitterFamille(){
    ...
    else { alert("⚠️ Impossible de quitter la famille pour le moment."); }
  }
  ...
  async function retirerInvite(i){
    ...
    if(res?.ok){
      ...
      alert("⚠️ Impossible de retirer l'invité pour le moment.");
    }
  }
