...
const TR = {
  fr:{
    ...
    installAppIos:"Appuyez sur le bouton Partager (▢↑) en bas de l'écran, puis sur « Sur l'écran d'accueil ».",
    installAppAndroidTitle:"Sur Android (Chrome)",
    installAppAndroid:"Appuyez sur le menu ⋮ en haut à droite, puis sur « Installer l'application » ou « Ajouter à l'écran d'accueil ».",
  ...
  en:{
    ...
    installAppIos:"Tap the Share button (▢↑) at the bottom of the screen, then tap \"Add to Home Screen\".",
    installAppAndroidTitle:"On Android (Chrome)",
    installAppAndroid:"Tap the ⋮ menu in the top right, then tap \"Install app\" or \"Add to Home screen\".",
  ...
  de:{
    ...
    installAppIos:"Tippen Sie unten auf das Teilen-Symbol (▢↑) und dann auf „Zum Home-Bildschirm“.",
    installAppAndroidTitle:"Auf Android (Chrome)",
    installAppAndroid:"Tippen Sie oben rechts auf das Menü ⋮ und dann auf „App installieren“ oder „Zum Startbildschirm hinzufügen“.",
  ...
  es:{
    ...
    installAppIos:"Toca el botón Compartir (▢↑) en la parte inferior y luego «Añadir a pantalla de inicio».",
    installAppAndroidTitle:"En Android (Chrome)",
    installAppAndroid:"Toca el menú ⋮ arriba a la derecha y luego «Instalar aplicación» o «Añadir a pantalla de inicio».",
  ...
  pt:{
    ...
    installAppIos:"Toque no botão Partilhar (▢↑) na parte inferior e, em seguida, em «Adicionar ao ecrã principal».",
    installAppAndroidTitle:"No Android (Chrome)",
    installAppAndroid:"Toque no menu ⋮ no canto superior direito e, em seguida, em «Instalar aplicação» ou «Adicionar ao ecrã principal».",
...
export default function App() {
  ...
  return (
    ...
    <div style={{display:"flex",flexDirection:"column",height:"100vh",maxWidth:940,margin:"0 auto",overflow:"hidden",width:"100%"}}>
          ...
          {showMenu && (
            ...
            <div style={{position:"fixed",top:62,right:14,background:C.card,border:`1.5px solid ${C.bor}`,borderRadius:16,minWidth:260,maxWidth:"90vw",zIndex:300,boxShadow:"0 12px 40px rgba(0,0,0,.2)",overflow:"hidden"}}>
              ...
              <div style={{padding:"10px 16px",borderTop:`1px solid ${C.bor}`,textAlign:"center",fontSize:10,color:C.mut,lineHeight:1.5}}>
                ...
                <a href="https://github.com/albertoramos78-a11y/duvia-app/blob/main/LICENSE" target="_blank" rel="noopener noreferrer" style={{color:C.vio,textDecoration:"underline"}}>{t.viewLicense}</a>
              </div>
            </div>
            </>
          )}
      ...
      {showInstallModal && (
        <div onClick={()=>setShowInstallModal(false)} style={{position:"fixed",inset:0,background:"rgba(0,0,0,.5)",zIndex:500,display:"flex",alignItems:"center",justifyContent:"center",padding:20}}>
          <div onClick={e=>e.stopPropagation()} style={{background:C.card,borderRadius:16,padding:20,maxWidth:400,width:"100%",boxShadow:"0 20px 60px rgba(0,0,0,.3)"}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
              ...
              <button onClick={()=>setShowInstallModal(false)} style={{width:30,height:30,background:C.sur,border:`1px solid ${C.bor}`,borderRadius:8,color:C.mut,fontSize:14,cursor:"pointer"}}>✕</button>
            </div>
            ...
            <div style={{background:C.sur,border:`1.5px solid ${C.bor}`,borderRadius:12,padding:"12px 14px",marginBottom:10}}>
              <div style={{fontSize:13,fontWeight:800,color:C.txt,marginBottom:4,display:"flex",alignItems:"center",gap:6}}>🍏 {t.installAppIosTitle}</div>
              <div style={{fontSize:12,color:C.mut,lineHeight:1.5}}>{t.installAppIos}</div>
            </div>

            <div style={{background:C.sur,border:`1.5px solid ${C.bor}`,borderRadius:12,padding:"12px 14px"}}>
              <div style={{fontSize:13,fontWeight:800,color:C.txt,marginBottom:4,display:"flex",alignItems:"center",gap:6}}>🤖 {t.installAppAndroidTitle}</div>
              <div style={{fontSize:12,color:C.mut,lineHeight:1.5}}>{t.installAppAndroid}</div>
            </div>
          </div>
        </div>
      )}
...
function LoginScreen({C,t,lang,setLang,themeMode,cycleTheme,users,setUsers,onLogin,onObsJoin,familySync}) {
  ...
  async function doLogin(){
    ...
    setOk("");
    setErr(`${t.wrongPw} (debug: ${res.error||"?"})`);
  }
