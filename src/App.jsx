...
import { APP_URL, LIMITS } from './config.js';
...
function useFamilySync(cfg, setCfg) {
    ...
    (async () => {
      try {
        ...
        try {
          ...
          console.warn("[Duvia][sync] membership lookup failed, falling back:", e);
        }
        ...
        // 2b. Sinon, on regarde la note locale (ancien comportement)
        if (!familyId) {
  ...
  async function refreshPendingMembers() {
    ...
    try {
      ...
      setPendingMembers(pendingRows.map(r => ({
        userId: r.user_id, role: r.role, joinedAt: r.joined_at, email: emailByUid[r.user_id] || null,
      })));
    } catch (e) {
  ...
  return { syncStatus, familyId, families, joinFamily, linkAccount, signInExisting, switchFamily, createNewFamily, refreshFamilies, joinFamilyByToken, pendingMembers, refreshPendingMembers, validateMember, rejectMember, pendingApproval };
}
...
export default function App() {
  ...
  const handleSetUser = useCallback((u) => {
    ...
    if (!u) {
      ...
      supabase.auth.signOut().catch(()=>{}).finally(()=>{ window.location.reload(); });
      return;
    }
    ...
    <div>
      ...
      {updateAvailable && (
        <div style={{position:"fixed",top:14,left:"50%",transform:"translateX(-50%)",zIndex:9999,
          ...
          <button onClick={()=>window.location.reload()} style={{padding:"6px 12px",background:C.vio,color:"#fff",fontSize:11,fontWeight:800,borderRadius:8}}>Rafraîchir</button>
        </div>
      )}
      {pendingUser ? (
      ...
      <div style={{padding:"0 14px",display:"flex",alignItems:"center",gap:12,height:58}}>
        ...
        {familySync.families.length > 1 && (
          <select
            ...
            style={{height:32,maxWidth:120,fontSize:11,fontWeight:700,color:C.txt,background:C.card,border:`1.5px solid ${C.bor}`,borderRadius:8,flexShrink:0}}
          >
...
function LoginScreen({C,t,lang,setLang,themeMode,cycleTheme,users,setUsers,onLogin,onObsJoin,familySync,cfg,setCfg}) {
  ...
  // Arrivée via lien d'invitation → afficher écran login par défaut
  useEffect(()=>{
    if(!isAnyInvite) return;
    const invEmail = obsInviteCode?.email || "";
    if(invEmail) setEmail(invEmail);
    // Toujours afficher login en premier — l'utilisateur peut basculer vers création de compte
    setMode("login");
  // eslint-disable-next-line react-hooks/exhaustive-deps
  },[]);
  ...
  async function doReg(){
    ...
    if(finalRole==="parent"){
      if(isParentInvite && obsInviteCode.newStyle){
        ...
        const joinRes = await familySync.joinFamilyByToken(obsInviteCode.code);
        if(joinRes.ok){
  ...
  function doForgot(){
    if(!email){setErr(t.allRequired);return;}
    if(isLikelyPhoneIdentifier(email.trim())){ setErr("La réinitialisation du mot de passe par téléphone n'est pas disponible. Contactez le parent qui vous a invité."); return; }
    setOk(users.find(u=>u.email===email)?t.resetSent:t.noAccount);setErr("");
  }
  ...
  async function doLoginAndJoin(){
    ...
    if(isObsInvite){
      ...
      if(obsInviteCode.newStyle){
        // 🔗 Nouveau format : se connecter, rejoindre en "pending", écran d'attente.
        const { error: p2Err } = await supabase.auth.signInWithPassword({ email: cleanEmail, password: pw });
        if(!p2Err){
          const joinRes = await familySync.joinFamilyByToken(obsInviteCode.code);
          if(joinRes.ok){
            setShowExistingAccount(false); setErr("");
...
function ConfigTab() {
  ...
  async function confirmInvite(){
    ...
    try {
      ...
      const { data: inv, error } = await supabase
        .from("family_invitations")
        .insert({ family_id: familySync.familyId, created_by: userData?.user?.id, role: "parent", email: em || null })
        .select("token")
        .single();
  ...
  function executeDeletion(i){
    ...
    // 🔧 Idem : sender_name est déjà figé sur chaque message dès l'envoi,
    // pas besoin de les ré-annoter ici.
...
function StepAccess() {
  ...
  useEffect(()=>{ familySync.refreshPendingMembers(); },[]);
      ...
      <div className="card" style={{marginBottom:16}}>
        {familySync.pendingMembers.length===0 ? (
          ...
          <div key={m.userId} style={{display:"flex",alignItems:"center",gap:10,padding:"8px 0",borderBottom:`1px solid ${C.bor}`}}>
            ...
            <div style={{flex:1,minWidth:0}}>
              <div style={{fontWeight:800,fontSize:13}}>{m.email || "Parent invité"}</div>
            ...
            <button disabled={pendingActionId===m.userId} onClick={async ()=>{
              ...
              const res = await familySync.validateMember(m.userId);
