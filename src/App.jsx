  ...
  async function doLoginAndJoin(){
    ...
    if(isObsInvite){
      ...
      if(obsInviteCode.newStyle){
        // 🔗 Nouveau format. On regarde mon statut RÉEL dans la famille :
        //  - déjà ACTIF (validé par le créateur) → j'entre dans l'app ;
        //  - en ATTENTE (déjà rejoint) → écran d'attente ;
        //  - pas encore membre → je rejoins puis écran d'attente.
        let myStatus = null;
        try {
          const { data: mems } = await supabase
            .from("family_members").select("status").eq("user_id", data.user.id);
          if((mems||[]).some(m=>m.status==="active")) myStatus="active";
          else if((mems||[]).some(m=>m.status==="pending")) myStatus="pending";
        } catch {}

        if(myStatus==="active"){
          // Validée → entrer dans l'app (rechargement pour charger la famille).
          setShowExistingAccount(false); setErr("");
          onLogin(updatedUser);
          duviaReload();
          return;
        ...
        if(myStatus!=="pending"){
          // Pas encore membre → rejoindre maintenant.
          await familySync.joinFamilyByToken(obsInviteCode.code, { name: u.name||updatedUser.name, gender: u.gender||meta.gender||"M" });
        }
        setShowExistingAccount(false); setErr("");
        setMode("obs_waiting");
        return;
