...
function useFamilySync(cfg, setCfg) {
  ...
  return { syncStatus, familyId, joinFamily, linkAccount, signInExisting };
}
...
function LoginScreen({C,t,lang,setLang,themeMode,cycleTheme,users,setUsers,onLogin,onObsJoin,familySync}) {
  ...
  async function doReg(){
    ...
    if(isObsInvite){
      onObsJoin({id:newId,name,email:cleanEmail,phone:regPhoneId||undefined,role:obsInviteCode.role||"grandparent",status:"pending",inviteCode:obsInviteCode.code});
...
function StepAccess() {
  ...
  function sendInvite(){
    ...
    setCfg(c=>({...c,pendingInvites:[...(c.pendingInvites||[]),{code,email,phone,role,canGuard,createdAt:new Date().toISOString(),used:false}]}));
    setSent(inviteUrl);
    setLastCode(code);
  }
