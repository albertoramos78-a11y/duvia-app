...
function LoginScreen({C,t,lang,setLang,themeMode,cycleTheme,users,setUsers,onLogin,onObsJoin,familySync}) {
  ...
  async function doLogin(){
    ...
    const { data: adminRow, error: adminErr } = await supabase
      ...
      .maybeSingle();
    if (adminErr) {
      // 🩺 Diagnostic temporaire — à retirer une fois le souci identifié.
      alert("Diagnostic admin — erreur: " + adminErr.message + " (code: " + (adminErr.code||"?") + ")");
    } else if (!adminRow) {
      alert("Diagnostic admin — aucune ligne trouvée pour uid: " + data.user.id);
    }
    if (adminRow) {
      setOk("");
